 ...
 * Muestra solo las órdenes asignadas al mecánico autenticado.
 */
...
const STATUS_LABEL: Record<string, string> = {
  ...
  READY_FOR_APPOINTMENT: 'Esperando llegada',
}
...
export default async function MecanicoPage() {
  const mechanicId = cookies().get('mechanic-session')?.value ?? ''

  // Mecánicos reales (de DB) filtran por asignación
  // El ID de fallback "demo-mechanic-id" muestra todas las órdenes
  const isRealMechanic = mechanicId && mechanicId !== 'demo-mechanic-id'

  const [mechanic, orders] = await Promise.all([
    // Buscar nombre del mecánico en DB
    isRealMechanic
    ...
    // Órdenes filtradas o todas (fallback demo)
    prisma.workOrder.findMany({
      where: {
        status: { in: ['READY_FOR_APPOINTMENT', 'VEHICLE_RECEIVED', 'IN_PROGRESS', 'QUALITY_CONTROL'] },
        ...(isRealMechanic ? { assignedMechanicId: mechanicId } : {}),
      },
      include: {
  ...
  const mecName   = mechanic ? `${mechanic.firstName} ${mechanic.lastName}` : 'Demo'
    ...
    <div className="min-h-screen bg-steel-900 text-gray-100 font-body pb-24">
      ...
      <header className="sticky top-0 z-10 bg-steel-800 border-b border-steel-600 px-4 py-3 flex items-center justify-between">
        <div>
          ...
          <h1 className="font-display text-xl font-bold text-brand leading-none">
            {mecName.toUpperCase()}
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <p className="text-brand font-bold text-sm">{orders.length} asignadas</p>
          </div>
          <LogoutButton />
        </div>
      </header>
      ...
      <main className="px-4 py-4 space-y-3">
        {orders.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-lg">Sin órdenes asignadas</p>
            <p className="text-gray-600 text-sm mt-1">
              {isRealMechanic
                ? 'El admin te asignará órdenes cuando haya trabajo'
